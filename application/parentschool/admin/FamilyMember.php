...
class FamilyMember extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = FamilyMemberModel::where($map)
            ->order($order)
            ->paginate();
...
//            ->setPageTips("总数量：" . $num2, 'danger')
            ->addTopButton("add")
            ->setPageTitle('列表')
            ->setSearch(['id' => 'ID', "student_id" => "学生id", 'uid' => 'uid']) // 设置搜索参数
            ->addOrder('id')
            ->addColumns([
                ['id', 'ID'],
                ['uid', '用户id', 'number'],
    ...
    public function edit($id = null)
    {
        ...
        // 获取数据
        $info = FamilyMemberModel::where('id', $id)
            ->find();
        $role = FamilyRoleModel::column("id,name");
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === FamilyMemberModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = FamilyMemberModel::where("id", $id)
