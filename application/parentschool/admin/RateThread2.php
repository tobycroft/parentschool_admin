...
class RateThread2 extends Admin
{
    ...
    public function export($ids = [])
    {
        ...
        $data_list = RateThreadModel::alias('a')->leftJoin(['ps_student' => 'b'], 'b.id=a.student_id')->where("length(content)>10 and img0<>'null'")
            ->where('a.id', 'in', $ids)
            ->order('a.id desc')
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = RateThreadModel::alias('a')->leftJoin(['ps_student' => 'b'], 'b.id=a.student_id')->where($map)->where("length(content)>10 and img0<>'null'")
            ->where($map)
            ->order($order)
            ->field("b.*,a.*")
...
//
        $num1 = RateThreadModel::where("date", ">", $todaytime)
            ->count();
...
//            ->addColumn('comment', '评论', 'number')
    ...
    public function edit($id = null)
    {
        ...
        // 获取数据
        $info = RateThreadModel::where('id', $id)
            ->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === RateThreadModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
