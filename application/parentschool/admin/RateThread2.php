...
class RateThread2 extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = RateThreadModel::alias('a')->leftJoin(['ps_student' => 'b'], 'b.id=a.student_id')->where($map)->where("length(content)>20 and img0<>'null'")
            ->where($map)
            ->order($order)
            ->field("b.*,a.*")
...
//            ->addColumn('comment', '评论', 'number')
            ->addColumn('hot_type', '选中热门类型', 'select', ['comment' => "评价", 'img' => '图片', 'all' => '图文评价', '' => '无'])
