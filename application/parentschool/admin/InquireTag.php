...
class InquireTag extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = InquireTagModel::where($map)
            ->order($order)
            ->paginate();
        $page = $data_list->render();
        $todaytime = date('Y-m-d H:i:s', strtotime(date("Y-m-d"), time()));
        $num1 = InquireTagModel::where("date", ">", $todaytime)->count();
        $num2 = InquireTagModel::count();
        $btn_access3 = [
            'title' => '列出题目',
            'icon' => 'fa fa-list',
...
//            ->addRightButton('delete') //添加删除按钮
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = InquireTagModel::create($data)) {
        ...
        return ZBuilder::make('form')
            ...
            ->addFormItems([ // 批量添加表单项
                ...
                ['select', 'type', '题目类型', 'single', InquireTypeModel::column('type,name'), 'single', "single"],
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (InquireTagModel::update($data)) {
                $user = InquireTagModel::get($data['id']);
                // 记录行为
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === InquireTagModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = InquireTagModel::where("id", $id)
