...
//            'class' => 'btn btn-xs btn-default ajax-get',
            'href' => url('http://api.ps.familyeducation.org.cn/v1/parent/wechat/create?data={"school_id":__school_id__}')
        ];
...
//            ->setPageTips("总数量：" . $num2, 'danger')
            ...
            ->addRightButtons(["生成二维码" => $btn_access3])
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = TeacherClassModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (TeacherClassModel::update($data)) {
                $user = TeacherClassModel::get($data['id']);
                // 记录行为
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === TeacherClassModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === TeacherClassModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === TeacherClassModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = TeacherClassModel::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            action_log('user_edit', 'user', $id, UID);
            $this->success('操作成功');
        } else {
