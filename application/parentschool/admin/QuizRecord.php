...
class QuizRecord extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = QuizRecordModel::where($map)
            ->order($order)
            ->paginate();
        $page = $data_list->render();
        $todaytime = date('Y-m-d H:i:s', strtotime(date("Y-m-d"), time()));

        $num1 = QuizRecordModel::where("date", ">", $todaytime)
            ->count();
...
//            ->addTopButton("add")
    ...
    public function edit($id = null)
    {
        ...
        // 获取数据
        $info = QuizRecordModel::where('id', $id)
            ->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === QuizRecordModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = QuizRecordModel::where("id", $id)
