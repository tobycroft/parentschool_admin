 ...
 */
class FamilyType extends Admin
{
    /**
     * 用户首页
     * @return mixed
...
//            ->setPageTips("总数量：" . $num2, 'danger')
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = FamilyTypeModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (FamilyTypeModel::update($data)) {
                $user = FamilyTypeModel::get($data['id']);
                // 记录行为
        ...
        // 获取数据
        $info = FamilyTypeModel::where('id', $id)->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === FamilyTypeModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === FamilyTypeModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === FamilyTypeModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = FamilyTypeModel::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            action_log('user_edit', 'user', $id, UID);
            $this->success('操作成功');
        } else {
