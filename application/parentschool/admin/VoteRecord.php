...
use app\parentschool\model\VoteRecordModel;
 ...
 */
class VoteRecord extends Admin
{
    /**
     * 用户首页
     * @return mixed
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = VoteRecordModel::where($map)
            ->order($order)
            ->paginate();
        $page = $data_list->render();
//        $todaytime = date('Y-m-d H:i:s', strtotime(date("Y-m-d"), time()));

//        $num1 = VoteRecordModel::where("date", ">", $todaytime)->count();
//        $num2 = VoteRecordModel::count();
...
//            ->setPageTips("总数量：" . $num2, 'danger')
            ...
            ->addColumn('icon', '区域图标', 'img_url')
            ->addColumn('img', '区域宣传图', 'img_url')
            ->addColumn('bg_img', '区域背景图', 'img_url')
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = VoteRecordModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (VoteRecordModel::update($data)) {
                $user = VoteRecordModel::get($data['id']);
                // 记录行为
        ...
        // 获取数据
        $info = VoteRecordModel::where('id', $id)
            ->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === VoteRecordModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = VoteRecordModel::where("id", $id)
