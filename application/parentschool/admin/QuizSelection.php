...
class QuizSelection extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = QuizSelectionModel::where($map)
            ->order($order)
            ->paginate();
...
//            ->setPageTips("总数量：" . $num2, 'danger')
            ...
            ->addColumn('remark', '错误提示', 'text.edit')
    ...
    public function edit($id = null)
    {
        ...
        // 获取数据
        $info = QuizSelectionModel::where('id', $id)
            ->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === QuizSelectionModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = QuizSelectionModel::where("id", $id)
