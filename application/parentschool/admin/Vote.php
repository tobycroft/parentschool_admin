...
use app\parentschool\model\VoteModel;
...
class Vote extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = VoteModel::where($map)
            ->order($order)
            ->paginate();
        $page = $data_list->render();
//        $todaytime = date('Y-m-d H:i:s', strtotime(date("Y-m-d"), time()));

//        $num1 = VoteModel::where("date", ">", $todaytime)->count();
//        $num2 = VoteModel::count();
...
//            ->setPageTips("总数量：" . $num2, 'danger')
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === VoteModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
