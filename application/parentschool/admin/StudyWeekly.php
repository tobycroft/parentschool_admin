...
class StudyWeekly extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = StudyWeeklyModel::where($map)
            ->order($order)
            ->paginate()
        ...
        $num1 = StudyWeeklyModel::where("date", ">", $todaytime)
            ->count();
...
//                "show_to" => $data["show_to"],
        ...
        return ZBuilder::make('form')
            ...
            ->addFormItems([ // 批量添加表单项
                ...
                ['ueditor', 'howto', '实践方法'],
                ['ueditor', 'notify', '特别提醒'],
                ['switch', 'can_show', '是否可以展示', '', '1'],
                ['switch', 'can_push', '是否可以推送', '', '1'],
                ['datetime', 'push_date', '推送日期', '', 'Y/m/d H:i:s'],
                ['datetime', 'show_date', '展示日期', '', 'Y/m/d H:i:s'],
...
//                "grade" => $data["grade"],
        ...
        $info2 = StudyModel::where("study_type", "weekly")
            ->where("study_id", $id)
            ->find();
        if (!$info2) {
            $study_input = [
        ...
        $info = StudyWeeklyModel::field("b.*,a.*")
            ->alias("a")
            ->leftJoin(["ps_study" => "b"], "b.study_id=a.id")
            ->where("b.study_type", "weekly")
            ->where('a.id', $id)
            ->find();
...
//                ['text', 'show_to', '展示给谁', "填写爸爸妈妈爷爷奶奶"],
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === StudyWeeklyModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = StudyWeeklyModel::where("id", $id)
