...
class StudyWeekly extends Admin
{
    ...
    public function index()
    {
        ...
        $data_list = StudyWeeklyModel::where($map)->order($order)->paginate()->each(function ($item, $key) {
            $item["common_tag"] = StudyTagModel::alias("a")->leftJoin(["ps_tag" => "b"], "a.tag_id=b.id")->where("study_id", $item["id"])->where("study_type", "weekly")->where("b.tag_type", "common")->column("name");
            $item["special_tag"] = StudyTagModel::alias("a")->leftJoin(["ps_tag" => "b"], "a.tag_id=b.id")->where("study_id", $item["id"])->where("study_type", "weekly")->where("b.tag_type", "special")->column("name");
