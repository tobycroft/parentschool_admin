...
use app\parentschool\model\SchoolAdminModel;
...
class SchoolAdmin extends Admin
{
    ...
    public function index()
    {
        ...
        $num1 = SchoolAdminModel::where("date", ">", $todaytime)->count();
        $num2 = SchoolAdminModel::count();
...
//            ->setPageTips("总数量：" . $num2, 'danger')
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = SchoolAdminModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        // 获取数据
        $info = SchoolAdminModel::where('id', $id)->find();
        $school = SchoolModel::column("id,name");
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === SchoolAdminModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === SchoolAdminModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === SchoolAdminModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = SchoolAdminModel::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            action_log('user_edit', 'user', $id, UID);
            $this->success('操作成功');
        } else {
