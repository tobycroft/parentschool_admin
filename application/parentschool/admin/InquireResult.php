 ...
 */
class InquireResult extends Admin
{
    /**
     * 用户首页
     * @return mixed
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = InquireResultModel::where($map)
            ->order($order)
            ->paginate();
        $page = $data_list->render();
        $todaytime = date('Y-m-d H:i:s', strtotime(date("Y-m-d"), time()));
        $num1 = InquireResultModel::where("date", ">", $todaytime)->count();
        $num2 = InquireResultModel::count();
        $btn_access3 = [
            'title' => '列出题目',
            'icon' => 'fa fa-list',
...
//            ->addRightButton('delete') //添加删除按钮
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = InquireResultModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        // 获取数据
        $info = InquireResultModel::where('id', $id)
            ->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === InquireResultModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
