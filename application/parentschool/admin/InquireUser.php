...
use app\parentschool\model\InquireUserModel;
 ...
 */
class InquireUser extends Admin
{
    /**
     * 用户首页
     * @return mixed
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = InquireUserModel::where($map)
            ->order($order)
            ->paginate();
        $page = $data_list->render();
        $todaytime = date('Y-m-d H:i:s', strtotime(date("Y-m-d"), time()));
        $num1 = InquireUserModel::where("date", ">", $todaytime)->count();
        $num2 = InquireUserModel::count();
        $btn_access3 = [
            'title' => '列出题目',
            'icon' => 'fa fa-list',
...
//            ->addRightButton('delete') //添加删除按钮
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($user = InquireUserModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (InquireUserModel::update($data)) {
                $user = InquireUserModel::get($data['id']);
                // 记录行为
        ...
        // 获取数据
        $info = InquireUserModel::where('id', $id)
            ->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === InquireUserModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = InquireUserModel::where("id", $id)
