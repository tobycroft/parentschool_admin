 ...
 */
class StudyMonthyTopic extends Admin
{
    /**
     * 用户首页
     * @return mixed
    ...
    public function index()
    {
        ...
        $num1 = StudyMonthyTopicModel::where("date", ">", $todaytime)->count();
        $num2 = StudyMonthyTopicModel::count();
...
//                ['number', 'monthy_id', '课程id', 'number'],
            ...
            ->assign([
                'file_upload_url' => "https://upload.tuuz.cc:444/v1/file/index/upload?token=ps"
            ])
...
//            ])
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === StudyMonthyTopicModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === StudyMonthyTopicModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === StudyMonthyTopicModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = StudyMonthyTopicModel::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            action_log('user_edit', 'user', $id, UID);
            $this->success('操作成功');
        } else {
