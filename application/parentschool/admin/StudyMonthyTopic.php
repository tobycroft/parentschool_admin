...
class StudyMonthyTopic extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 读取用户数据
        $data_list = StudyMonthyTopicModel::where($map)
            ->order($order)
            ->paginate();
        $page = $data_list->render();
        $todaytime = date('Y-m-d H:i:s', strtotime(date("Y-m-d"), time()));

        $num1 = StudyMonthyTopicModel::where("date", ">", $todaytime)
            ->count();
...
//                ['number', 'monthy_id', '课程id', 'number'],
    ...
    public function edit($id = null)
    {
        ...
        // 获取数据
        $info = StudyMonthyTopicModel::where('id', $id)
            ->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === StudyMonthyTopicModel::where('id', 'in', $ids)
                        ->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = StudyMonthyTopicModel::where("id", $id)
